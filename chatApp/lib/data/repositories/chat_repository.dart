...
class ChatRepository extends BaseRepository {
  ...
  /// Debug helper: run the chatRooms query once to force Firestore to throw the
  /// "requires an index" FirebaseException which contains the create-index URL.
  /// Call this method from your app (for example after login) and watch the
  /// debug console output. The exception message will include the prefilled
  /// Firebase Console URL you can open to create the required composite index.
  Future<void> runIndexProbeForChatRooms(String userId) async {
    try {
      ...
      print('Index probe succeeded — returned ${snapshot.docs.length} doc(s).');
    } on FirebaseException catch (e) {
      // e.message typically contains the 'create index' URL. Print the whole
      // exception so the URL is easy to copy/open.
      print('FirebaseException during index probe: ${e.message}');
      print('Full exception: $e');
    } catch (e, st) {
      print('Unexpected error during index probe: $e\n$st');
    }
  }
